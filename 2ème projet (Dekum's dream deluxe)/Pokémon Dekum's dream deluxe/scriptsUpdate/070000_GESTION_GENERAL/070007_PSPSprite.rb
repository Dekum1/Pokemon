...
#
# Par FL0RENT
# Corrigé par Metaiko et Damien Linux
# 29/12/2019
#---------------------------------------------------

MINIMUM_BATTLER_SIZE = 96
  
MINIMUM_PKMN_ICON_SIZE = 48

module POKEMON_S
  class PSP_Sprite < RPG::Sprite
    attr_accessor :base_zoom
    attr_reader :true_zoom_x
    attr_reader :true_zoom_y
    @@_animations = []
    @@_reference_count = {}
    ...
    def set_base_zoom(n)
      @base_zoom = n
      self.zoom_x = @true_zoom_x
      self.zoom_y = @true_zoom_y
    end
    ...
    def animation_set_sprites(sprites, cell_data, position)
      ...
      if not jump
        ...
        if not @_reverse
          if position == 3
            self.x += (cell_data[15, 1] - 144)
            self.y += (cell_data[15, 2] + 72)
            
          else
            self.x += cell_data[15, 1]
            self.y += cell_data[15, 2]
            x_compensate -= cell_data[15, 1] - 144 if position != 3
            y_compensate -= cell_data[15, 2] + 72 if position != 3
          end
      ...
      0.upto(14) do |i|
        ...
        if position == 3
          if self.viewport != nil
            ...
            sprite.x = @_reverse ? @_registered_x:320
            sprite.y = @_reverse ? @_registered_y:240    
          end
        else
          ...
          sprite.y += 32 if @_registered_y < 240 and $battle_var.in_battle
        end
        if position == 3
          if not @_reverse
            sprite.x += (cell_data[i, 1]) + x_compensate#- 144) + x_compensate
            sprite.y += (cell_data[i, 2]) + y_compensate# + 72) + y_compensate
          else
            sprite.x -= (cell_data[i, 1] - 144) - x_compensate
            sprite.y -= (cell_data[i, 2] + 72) - y_compensate
          end
        else
          if not @_reverse
